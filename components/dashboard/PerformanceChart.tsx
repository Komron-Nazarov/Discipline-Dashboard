...
export default function PerformanceChart({ transactions, balanceLabel = "Balance", emptyLabel = "Add a transaction to build your chart", locale = "en" }: { transactions: Transaction[]; balanceLabel?: string; emptyLabel?: string; locale?: "ru" | "en" }) {
  const data = transactions.reduce<{ step: string; balance: number }[]>((points, tx, index) => {
  ...
  if (!data.length) {
    return <div className="grid h-52 w-full place-items-center sm:h-56"><div className="text-center"><span className="mx-auto block h-px w-16 bg-[#d8ff3e]/35" /><p className="mt-4 max-w-60 text-[11px] leading-5 text-[#666960]">{emptyLabel}</p></div></div>;
  }
