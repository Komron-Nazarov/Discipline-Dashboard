...
function About({ onClose, onGuide }: { onClose: () => void; onGuide: () => void }) {
  ...
  return <div className="fixed inset-0 z-[80] grid place-items-center bg-black/70 p-4 backdrop-blur-sm"><section className="fade-up relative w-full max-w-md rounded-[20px] border border-white/10 bg-[#11120f] p-5 shadow-2xl sm:p-7" role="dialog" aria-modal="true" aria-label={t("about.title")}>
    ...
    <div className="mt-4 flex items-center gap-3 rounded-2xl border border-[#d8ff3e]/20 bg-[#d8ff3e]/6 p-3.5"><Image src="/kn-logo-source.png" width={48} height={48} alt="Komron Nazarov" className="creator-logo size-12 shrink-0" /><div><p className="text-[13px] font-bold">{t("about.credit")}</p><p className="mt-1 text-[9px] text-[#6f7269]">{t("about.personalMark")}</p></div></div>
...
function CreatorSignature({ onAbout }: { onAbout: () => void }) {
  ...
  return <footer className="mt-8 border-t border-white/8 py-6">
    <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-center">
      <button onClick={onAbout} className="group flex items-center gap-3 text-left"><Image src="/kn-logo-source.png" width={48} height={48} alt="Komron Nazarov" className="creator-logo size-12 transition group-hover:opacity-80" /><span><span className="eyebrow block text-[7px]">{t("creator.kicker")}</span><strong className="mt-1 block text-[13px]">{t("creator.name")}</strong></span></button>
